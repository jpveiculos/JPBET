...
function criarSessaoJogador(userId) {
  const token = criarTokenJogador();
  playerSessions.set(token, {
    userId: Number(userId),
    expiresAt:
      Date.now() + PLAYER_SESSION_TTL
  });
  return token;
}
function obterSessaoJogador(token) {
  ...
  const sessao = playerSessions.get(token);
  if (!sessao) {
    return null;
  }
...
function verificarPassword(
  ...
  /*
    ...
    que ainda tenham a senha armazenada
    em texto simples.
  */
  return (
