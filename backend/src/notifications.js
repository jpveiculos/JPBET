...
function gerarChavesVapid() {
  ...
  return {
    ...
    privateKey: base64url(Buffer.from(priv.d, "base64url"))
  };
}

async function garantirInfraPush() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS admin_push_config (
  ...
  if (!existing.rows.length) {
    ...
    await pool.query(
      `INSERT INTO admin_push_config(id,public_key,private_key,subject) VALUES(1,$1,$2,$3)`,
      [keys.publicKey, keys.privateKey, "https://jp-bet.onrender.com/"]
    );
  }
}
...
export async function obterChavePublicaPush() {
  const config = await obterPushConfig();
...
export async function salvarAssinaturaPush(subscription) {
  ...
  await pool.query(
    `INSERT INTO admin_push_subscriptions(endpoint,subscription_json,updated_at)
     ...
     ON CONFLICT(endpoint) DO UPDATE SET subscription_json=EXCLUDED.subscription_json,updated_at=CURRENT_TIMESTAMP`,
    [endpoint, JSON.stringify({ endpoint, keys: { p256dh, auth } })]
  );
  return { ok: true };
}
...
export async function removerAssinaturaPush(endpoint) {
  ...
  await garantirInfraPush();
  await pool.query(`DELETE FROM admin_push_subscriptions WHERE endpoint=$1`, [value]);
  return { ok: true };
}
  ...
  for (const row of result.rows) {
    try {
      ...
      if (status === 404 || status === 410) {
        ...
        console.error("Falha no push do MyBets:", status || error?.message || error);
      }
    }
  }
...
export async function inicializarNotificacoes() {
  try {
    ...
    console.error("Erro ao inicializar infraestrutura de push MyBets:", error);
  }
}
