...
function criarRodaMyBets(){
 ...
 svg.innerHTML='';
 const base=document.createElementNS(ns,'circle');base.setAttribute('cx',cx);base.setAttribute('cy',cy);base.setAttribute('r',rad);base.setAttribute('fill','#d39b21');svg.appendChild(base);
 for(let i=0;i<16;i++){
  const label=String(seg[i]||'').trim();if(!label)continue;const s=g[i];
  const path=document.createElementNS(ns,'path');path.setAttribute('d',sectorPath(cx,cy,rad,s.startDeg,s.endDeg));path.setAttribute('fill','#050505');path.setAttribute('stroke','#050505');path.setAttribute('stroke-width','0');svg.appendChild(path);
  const p=polar(cx,cy,150,s.centerDeg),text=document.createElementNS(ns,'text');text.textContent=label.toUpperCase();text.setAttribute('x',p.x);text.setAttribute('y',p.y);text.setAttribute('class','roulette-label multiplier');text.setAttribute('font-size','58');text.setAttribute('fill','#25e66b');text.style.fill='#25e66b';text.dataset.x=p.x;text.dataset.y=p.y;svg.appendChild(text);
 }
...
.mybets-profile-extra{width:46px!important;min-width:46px!important;height:46px!important;border-radius:50%!important;font-size:25px!important;color:#fff!important;}
...
.roulette-balance{position:absolute!important;top:14px!important;right:14px!important;left:auto!important;z-index:90!important;display:flex!important;align-items:center!important;gap:7px!important;padding:8px 12px!important;border:1px solid rgba(245,197,66,.55)!important;border-radius:15px!important;background:rgba(7,9,14,.96)!important;color:#a5adbb!important;font:800 13px Arial!important;}
.roulette-balance strong{color:#ffdf75!important;font-size:16px!important;}
...
`;
 ...
 if(header){
  ...
  if(!hr.querySelector('.mybets-profile-extra')){const profile=document.createElement('button');profile.type='button';profile.className='menu-button mybets-profile-extra';profile.textContent='●';profile.setAttribute('aria-label','Perfil');hr.appendChild(profile);}
 }
 ...
 const sync=()=>{const el=document.getElementById('rouletteBalance'),hb=document.querySelector('.header-balance');let v=0;try{v=typeof obterSaldo==='function'?obterSaldo():(window.usuarioAtual?.balance||0);}catch(_){v=0;}const txt='R$ '+Number(v||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2});if(el)el.textContent=txt;if(hb)hb.textContent='Saldo  '+txt;};
 sync();
