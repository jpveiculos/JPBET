...
function atualizarSaldos(v) {
  ...
  if (balance) balance.textContent = valor.toLocaleString("pt-BR", {minimumFractionDigits:2, maximumFractionDigits:2});
  if (stageBalance) stageBalance.textContent = valor.toLocaleString("pt-BR", {minimumFractionDigits:2, maximumFractionDigits:2});
}

function configurarBotoesVoltar() {
  const backButton = $("backButton");
  const stageBack = $("stageBack");
  if (backButton) backButton.onclick = () => { window.location.href = "dashboard.html"; };
  if (stageBack) stageBack.onclick = voltarLobby;
}

/* A Central é pública e não depende de /api/games. */
function catalogoLocalDeJogos() {
  return [
...
async function carregarJogos() {
  jogos = catalogoLocalDeJogos();
  renderizarJogos();

  const id = new URLSearchParams(location.search).get("game");
  if (id) await abrirJogo(id);
}
...
function renderizarJogos() {
  ...
  if (!box) return;
  if (message) message.style.display = jogos.length ? "none" : "block";
...
function normalizarIdJogo(gameId) {
  ...
  const aliases = {
    diamondgold:"diamondGold",
    royaljackpot:"royalJackpot"
  };
...
async function abrirJogo(gameId) {
  try {
    ...
    const config = catalogoLocalDeJogos().find(j => String(j.id).toLowerCase() === id.toLowerCase());
    ...
    if (id === "roulette") abrirInterfaceRoleta(config);
...
function criarRoletaSorte() {
  ...
  if (!wheel) return;
  const quantidade = ROULETTE_VISUAL_SEGMENTS.length;
  const angle = 360 / quantidade;
  const colors = ROULETTE_VISUAL_SEGMENTS.map((_, i) => i % 2 === 0 ? "#D4AF37" : "#111111");
  wheel.innerHTML = "";
  ROULETTE_VISUAL_SEGMENTS.forEach((seg, i) => {
    const d = document.createElement("span");
    d.className = `roulette-label ${seg.type === "zero" ? "zero" : "prize"}`;
    d.textContent = seg.label;
    const mid = i * angle + angle / 2;
    const radius = Math.min(43, Math.max(36, (wheel.clientWidth || 300) * 0.14));
    d.style.transform = `translate(-50%,-50%) rotate(${mid}deg) translateY(-${radius}%) rotate(${-mid}deg)`;
    d.dataset.index = i;
    wheel.appendChild(d);
  });
  const faixas = colors.map((color, i) => `${color} ${i * angle}deg ${(i + 1) * angle}deg`);
  wheel.style.background = `repeating-conic-gradient(from -${angle / 2}deg, transparent 0deg ${angle - 1}deg, rgba(255,221,105,.85) ${angle - 1}deg ${angle}deg), conic-gradient(from -${angle / 2}deg, ${faixas.join(",")})`;
}
...
function atualizarGiroGratis() {
  ...
  if (qtd > 0 && aposta > 0) {
    box.hidden = false;
    box.textContent = `🍀 ${qtd} giro${qtd === 1 ? "" : "s"} grátis disponível${qtd === 1 ? "" : "eis"} — aposta ${moeda(aposta)}`;
    btn.textContent = "USAR GIRO GRÁTIS";
  } else {
    box.hidden = true;
    box.textContent = "";
...
function definirRouletteBet(v) {
  ...
  const max = Math.max(min, numero(configuracaoAtual?.maxBet, 100));
...
async function girarSlot() {
  ...
  if (bet < numero(configuracaoAtual?.minBet, 1) || bet > numero(configuracaoAtual?.maxBet, 1000)) return mostrarToast("Aposta fora dos limites.");
  ...
  try {
    const resposta = await fetch(`${API}/games/spin`, {method:"POST", headers:headersJSON(), body:JSON.stringify({userId, gameId:configuracaoAtual.id, bet})});
    ...
    if (win > 0) { cabinet.classList.add("win"); $("slotStatus").textContent = `Você ganhou ${moeda(win)}!`; }
    else $("slotStatus").textContent = "Boa sorte no próximo giro!";
  } catch(e) {
    $("slotStatus").textContent = "Boa sorte!";
...
async function girarRoleta() {
  ...
  try {
    ...
    if (index < 0 || index >= ROULETTE_VISUAL_SEGMENTS.length) throw new Error("Resultado inválido.");
    const wheel = $("rouletteWheel");
    ...
    const animationMs = numero(configuracaoAtual?.animationMs,4800);
    ...
    $("rouletteResult").textContent = prize > 0 ? `🎉 ${seg.label} — Prêmio ${moeda(prize)}${hasFree ? " (giro grátis)" : ""}` : "❌ PERDEU — Prêmio R$ 0,00";
    criarRoletaSorte();
    atualizarGiroGratis();
  } catch(e) {
