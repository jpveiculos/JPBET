(function(){
  ...
  function renderWheel(){
    ...
    const step=45,textColors=['#ff8a22','#39a9ff','#31e56d','#d9d9df'];
    for(let v=0;v<8;v++){
      const a=v*step-step/2,b=(v+1)*step-step/2;
      const sector=document.createElementNS(ns,'path');sector.setAttribute('d',path(239,a,b));sector.setAttribute('fill',v%2===0?'#050505':'#f7b915');sector.setAttribute('stroke','#080808');sector.setAttribute('stroke-width','2');svg.appendChild(sector);
      if(v%2===0){
        const pos=v/2,q=polar(172,v*step);
        const t=document.createElementNS(ns,'text');
        t.textContent=money(prizes[pos]);t.setAttribute('x',q.x);t.setAttribute('y',q.y);t.setAttribute('text-anchor','middle');t.setAttribute('dominant-baseline','middle');t.setAttribute('font-family','Arial Black,Arial,sans-serif');t.setAttribute('font-size',slices>=60?'22':'25');t.setAttribute('font-weight','900');t.setAttribute('fill',textColors[pos]);t.setAttribute('stroke','#050505');t.setAttribute('stroke-width','4');t.setAttribute('paint-order','stroke fill');t.setAttribute('style','writing-mode:horizontal-tb;glyph-orientation-vertical:0;');
        svg.appendChild(t);
      }
    }
    ...
    if($('wheel')){$('wheel').style.transformOrigin='50% 50%';$('wheel').style.transformBox='fill-box';$('wheel').style.transform=`rotate(${rotation}deg)`}
  }
  async function spin(){
    ...
    try{const r=await fetch(api,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:user.id,betAmount:bet,rouletteId:cfg.id})});const d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw Error(d.message||'Não foi possível realizar a rodada.');const s=d.spin||{};
      ...
      await new Promise(resolve=>{function frame(now){const p=Math.min(1,(now-start)/dur),e=1-Math.pow(1-p,3),rr=from+(dest-from)*e;$('wheel').style.transform=`rotate(${rr}deg)`;if(p<1)return requestAnimationFrame(frame);rotation=dest;$('wheel').style.transform=`rotate(${dest}deg)`;resolve()}requestAnimationFrame(frame)});
