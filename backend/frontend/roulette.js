...
function renderWheel(){
 ...
 const orange=document.createElementNS(ns,'linearGradient');orange.setAttribute('id','prizeOrange');orange.setAttribute('x1','0%');orange.setAttribute('y1','0%');orange.setAttribute('x2','0%');orange.setAttribute('y2','100%');orange.setAttribute('x2','0%');orange.setAttribute('y2','100%');orange.innerHTML='<stop offset="0%" stop-color="#ffcf80"/><stop offset="22%" stop-color="#ffad32"/><stop offset="65%" stop-color="#ff8500"/><stop offset="100%" stop-color="#b84b00"/>';defs.appendChild(orange);
...
async function spinRoulette(){
 if(spinning)return;const bet=Number($('betAmount').value),min=Number(settings.roulette_min_bet||1),max=Number(settings.roulette_max_bet||100);if(!(bet>=min&&bet<=max))return toast(`A aposta deve estar entre ${money(min)} e ${money(max)}.`,'error');if(bet>Number(user.balance||0))return toast('Saldo insuficiente.','error');spinning=true;$('spinButton').disabled=true;$('bottomSpinButton').disabled=true;$('spinButton').style.opacity='.6';
...
function configurarInterfaceRoleta(){
 ...
 removerRetorno();
 document.querySelectorAll('#bottomSpinButton,.spin-button').forEach(el=>{if(el.id==='bottomSpinButton')el.remove()});
