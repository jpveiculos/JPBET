...
if(location.pathname.endsWith('/roulette207.html')){const nativeRAF=window.requestAnimationFrame.bind(window);let rafBase=null;window.requestAnimationFrame=function(callback){return nativeRAF(function(realNow){if(window.spinning){if(rafBase===null)rafBase=realNow;const elapsed=realNow-rafBase;const p=Math.min(1,elapsed/8500);const originalProgress=p;callback(rafBase+originalProgress*5000);if(p>=1)rafBase=null}else{rafBase=null;callback(realNow)}})}})();

/* Ponteiro 3D vibrante da roleta */
if(location.pathname.endsWith('/roulette207.html')){const pointerStyle=document.createElement('style');pointerStyle.textContent=`.pointer{top:-7px!important;width:58px!important;height:78px!important;background:linear-gradient(105deg,#fffbd0 0%,#fff06a 10%,#ffd21f 27%,#ff9f00 58%,#b76500 82%,#6e3600 100%)!important;clip-path:polygon(50% 100%,5% 28%,8% 17%,23% 5%,39% 0,61% 0,77% 5%,92% 17%,95% 28%)!important;filter:drop-shadow(0 7px 5px rgba(0,0,0,.95)) drop-shadow(0 0 11px rgba(255,185,0,.65))!important;transform:translateX(-50%) perspective(120px) rotateX(-4deg)!important}.pointer::before{left:10px!important;right:10px!important;top:10px!important;bottom:13px!important;background:linear-gradient(180deg,#ff735f 0%,#ff3025 18%,#e50916 48%,#b0000b 76%,#650008 100%)!important;clip-path:polygon(50% 100%,0 0,100% 0)!important;border-radius:7px 7px 12px 12px!important;box-shadow:inset 0 2px 0 rgba(255,255,255,.72),inset 4px 0 8px rgba(255,255,255,.14),inset -5px 0 9px rgba(70,0,0,.45),0 3px 5px rgba(0,0,0,.45)!important}.pointer::after{top:5px!important;width:13px!important;height:13px!important;background:radial-gradient(circle at 30% 25%,#fff 0%,#fffbd0 22%,#ffe43b 48%,#ff9d00 72%,#8d4a00 100%)!important;box-shadow:0 0 0 2px #5e3500,0 0 0 4px rgba(255,194,0,.34),0 0 13px rgba(255,221,74,.95),inset 0 -2px 3px rgba(111,51,0,.5)!important}@media(max-width:600px){.pointer{width:51px!important;height:70px!important}.pointer::before{left:9px!important;right:9px!important;top:9px!important;bottom:12px!important}.pointer::after{width:11px!important;height:11px!important}}`;document.head.appendChild(pointerStyle);}`
})();