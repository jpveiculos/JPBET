...
async function verificarAdmin() {
  try {
    /*
      A rota /api/admin não existe no backend atual.
      Usamos /api/admin/settings porque ela é protegida
      pelo middleware administrativo e confirma se o
      cookie da sessão está válido.
    */
    ...
    /*
      IMPORTANTE:
      Não redirecionar automaticamente para /admin.
      Isso evita o loop de reinicialização.
    */
    return false;
  }
}
...
async function carregarDashboardAdmin() {
  ...
  try {
    ...
    console.error(error);
    /*
      A ausência da rota dashboard não deve
      expulsar o administrador do painel.
    */
    if (message) {
      message.textContent =
