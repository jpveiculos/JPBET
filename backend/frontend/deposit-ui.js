(() => {
  ...
  function qrUrl(code) {
    return "https://quickchart.io/qr?size=320&margin=2&ecLevel=M&text=" + encodeURIComponent(code || "");
  }
  ...
  async function carregarPix() {
    ...
    try {
      ...
      const s = d.settings || {};
      pix = { key: s.pix_key || "", keyType: s.pix_key_type || "", receiver: s.pix_receiver_name || "MyBets", city: s.pix_city || "Brasilia", description: s.pix_description || "MyBets" };
      loaded = true;
    } catch (_) {}
  }
  ...
  function atualizar() {
    ...
    if (!input || !img) return;
    const code = montarPix(Number(input.value || 0));
    img.src = code ? qrUrl(code) : qrUrl(montarPix(0));
    img.style.opacity = code ? "1" : ".45";
    img.title = code ? "QR Code Pix com o valor informado" : "Informe o valor para gerar o QR Code com o valor";
    img.dataset.pixCode = code;
  }

  async function copiar() {
    const code = document.getElementById("depositQr")?.dataset.pixCode || montarPix(0);
    const msg = document.getElementById("depositPixMessage");
    if (!code) { if (msg) msg.textContent = "A chave Pix ainda não está configurada."; return; }
    try { await navigator.clipboard.writeText(code); if (msg) msg.textContent = "✅ Código Pix copiado."; }
    catch (_) { if (msg) msg.textContent = "Não foi possível copiar automaticamente. Use o QR Code."; }
  }
